...
MyDonateQRWidget::MyDonateQRWidget(QWidget* parent) : QWidget(parent) {
  timer = new QTimer(this);
  connect(timer, &QTimer::timeout, this, &MyDonateQRWidget::refresh);
}

void MyDonateQRWidget::showEvent(QShowEvent *event) {
  refresh();
  timer->start(5 * 60 * 1000);
  device()->setOffroadBrightness(100);
}

void MyDonateQRWidget::hideEvent(QHideEvent *event) {
  timer->stop();
  device()->setOffroadBrightness(BACKLIGHT_OFFROAD);
}

void MyDonateQRWidget::refresh() {
  QString qrString = "https://www.paypal.com/donate/?hosted_button_id=JN4E8PTL4QBZG";
  this->updateQrCode(qrString);
  update();
}

void MyDonateQRWidget::updateQrCode(const QString &text) {
...
void MyDonateQRWidget::paintEvent(QPaintEvent *e) {
  QPainter p(this);
...
MyDonatePopup::MyDonatePopup(QWidget *parent) : DialogBase(parent) {
  QHBoxLayout *hlayout = new QHBoxLayout(this);
  hlayout->setContentsMargins(0, 0, 0, 0);
  hlayout->setSpacing(0);

  setStyleSheet("MyDonatePopup { background-color: #E0E0E0; }");
  ...
  hlayout->addLayout(vlayout, 1);
  {
    ...
    QLabel *title = new QLabel(tr("Keep this project alive!"), this);
    ...
    )").arg(tr("Scan the QR code on the right"))
    .arg(tr("Donate any value you feel in your heart")), this);
  ...
  // QR code
  MyDonateQRWidget *qr = new MyDonateQRWidget(this);
